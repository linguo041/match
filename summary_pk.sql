...
-- 			t.lose_pk_cnt / (t.win_pk_cnt + t.draw_pk_cnt + t.lose_pk_cnt)) win_draw_lose_pk_rate,
		...
		t.hot_point, t.host_att_to_guest, t.guest_att_to_host, t.host_att_variation_to_guest, t.guest_att_variation_to_host,
...
select m1.ofn_match_id, m1.league, m1.match_time, m1.host_name, m1.guest_name, concat(mr1.host_score, ' - ', mr1.guest_score) res,
		...
		mls1.hot_point, mls1.host_att_to_guest, mls1.guest_att_to_host, mls1.host_att_variation_to_guest, mls1.guest_att_variation_to_host,
	...
	where m1.ofn_match_id in (select ofn_match_id from matches
			where 1=1
            -- and cal_phase = 1
			and match_time > '2017-02-04 00:00:00'
			-- and match_time < '2017-01-15 00:00:00'
			-- and match_day_id is not null
			)
	  -- pk
	  -- and (mpk1.main_pk = mpk.main_pk or mpk1.current_pk = mpk.current_pk or mpk1.main_pk = mpk.current_pk or mpk.main_pk = mpk1.current_pk) 
	  ...
	  and abs(mpk.main_h_win - mpk1.main_h_win) <= 0.08
      and abs((mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - (mpk1.main_pk-(mpk1.main_h_win-mpk1.main_a_win)/2)) <= 0.15
	  and abs(mpk.current_h_win - mpk1.current_h_win) <= 0.06
	  and abs((mpk.current_pk-(mpk.current_h_win-mpk.current_a_win)/2) - (mpk1.current_pk-(mpk1.current_h_win-mpk1.current_a_win)/2)) <= 0.1
      and abs(mpk.home_win_change_rate - mpk1.home_win_change_rate) <= 0.08
	  -- state
	  and abs(mls.hot_point - mls1.hot_point) <= 4
      and (abs(mpk1.main_pk) <= 0.75 and abs((mls.host_att_to_guest - mls.guest_att_to_host) - (mls1.host_att_to_guest - mls1.guest_att_to_host)) < 0.3
		or abs(mpk1.main_pk) > 0.75 and abs((mls.host_att_to_guest - mls.guest_att_to_host) - (mls1.host_att_to_guest - mls1.guest_att_to_host)) < 0.48)
      -- base
      and (abs(mpk1.main_pk) <= 0.75 and abs((mcs.host_att_guest_def - mcs.guest_att_host_def) - (mcs1.host_att_guest_def - mcs1.guest_att_host_def)) < 0.28
		or abs(mpk1.main_pk) > 0.75 and abs((mcs.host_att_guest_def - mcs.guest_att_host_def) - (mcs1.host_att_guest_def - mcs1.guest_att_host_def)) < 0.4)
      and abs(mcs.host_level - mcs1.host_level) <= 1
	  and abs(mcs.guest_level - mcs1.guest_level) <= 1
	  -- js
      and (abs(mj.win_draw_pk_rate - mj1.win_draw_pk_rate) < 0.35 or abs(mj.win_draw_rate - mj1.win_draw_rate) < 0.35)
	  -- pl
      -- and (abs(mce1.main_win_pl - mce.main_win_pl) <= 0.15 or abs(mce1.main_lose_pl - mce.main_lose_pl) <= 0.15)
	  group by m1.ofn_match_id
      order by m1.match_time asc
) t 
 where 1=1
	-- and t.win_cnt + t.draw_cnt + t.lose_cnt > 3
 order by t.match_time asc, t.ofn_match_id asc
