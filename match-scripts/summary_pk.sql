...
select m1.ofn_match_id, m1.league, m1.match_time, m1.host_name, m1.guest_name, concat(mr1.host_score, ' - ', mr1.guest_score) res,
	...
	where m1.ofn_match_id in (select ofn_match_id from matches
      ...
      and m.cal_phase = 2
	  and abs(l1.goal_per_match - l.goal_per_match) <= 0.32 + abs(l1.goal_per_match - 2.68) * 0.7
      and l.state = 0
	  -- pk
      -- and (abs(mpk1.main_pk-0.2) <= 0.75
-- 			and abs((mp.predict_pk - mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - (mp1.predict_pk - mpk1.main_pk-(mpk1.main_h_win-mpk1.main_a_win)/2)) <= 0.2
...
-- 			and abs((mp.predict_pk - mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - (mp1.predict_pk - mpk1.main_pk-(mpk1.main_h_win-mpk1.main_a_win)/2)) <= 0.25))
	  and abs((mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - (mpk1.main_pk-(mpk1.main_h_win-mpk1.main_a_win)/2)) <= 0.07
      and abs((mpk.current_pk-(mpk.current_h_win-mpk.current_a_win)/2) - (mpk1.current_pk-(mpk1.current_h_win-mpk1.current_a_win)/2)) <= 0.05
      and abs(mpk1.current_h_win - mpk.current_h_win) <= 0.5
	  ...
	  and (mce1.current_win_pl < 2.601 and abs(mce1.current_win_pl - mce.current_win_pl) <= mce1.current_win_pl * 0.035
		or mce1.current_lose_pl < 2.601 and abs(mce1.current_lose_pl - mce.current_lose_pl) <= mce1.current_lose_pl * 0.035)
