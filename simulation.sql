...
-- 小球 走让球
select m.ofn_match_id, m.league, m.match_time, m.host_name, m.guest_name, mr.host_score, mr.guest_score,
		mcs.host_att_guest_def b_h_att, mcs.guest_att_host_def b_g_att,
        mp.predict_pk, cast(mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2 as decimal(5,3)) main,
        mpk.current_pk, mpk.current_h_win, mpk.home_win_change_rate,
        mce.win_change_per_hour, mce.draw_change_per_hour, mce.lose_change_per_hour,
		mls.hot_point, mls.host_att_to_guest l_h_att, mls.guest_att_to_host l_g_att,
		mls.host_att_variation_to_guest l_h_var, mls.guest_att_variation_to_host l_g_var,
		m.cal_phase
	from matches m
		left join match_club_state mcs on m.ofn_match_id = mcs.ofn_match_id
		left join match_latest_state mls on m.ofn_match_id = mls.ofn_match_id
		...
		left join match_predict mp on m.ofn_match_id = mp.ofn_match_id
    where 1=1
	  and mls.host_att_variation_to_guest + mls.guest_att_variation_to_host <= 2.70
      and mls.host_att_variation_to_guest < 1.6
      and mls.guest_att_variation_to_host < 1.6
      and mcs.host_att_guest_def + mcs.guest_att_host_def <= 2.7
      and abs(mcs.host_att_guest_def - mcs.guest_att_host_def) < 0.65
	  -- and mpk.current_h_win <= 0.96
      -- and (mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - mp.predict_pk > -0.1
      and m.league not in ('OuLian', 'OuGuan')
      and cal_phase = 2
     order by m.match_time desc;
...
select m.ofn_match_id, m.league, m.match_time, m.host_name, m.guest_name, mr.host_score, mr.guest_score,
    ...
    where 1=1
	  and mls.host_att_variation_to_guest + mls.guest_att_variation_to_host <= 2.70
      and mls.host_att_variation_to_guest <= 1.5
      and mls.guest_att_variation_to_host <= 1.5
	  -- and abs(mcs.host_att_variation_to_guest - mcs.guest_att_variation_to_host) < 0.45
	  and mls.host_att_to_guest + mls.guest_att_to_host <= 2.9
      and mls.host_att_to_guest < 1.7 * mls.guest_att_to_host
      and mls.host_att_to_guest > 0.58 * mls.guest_att_to_host
      and (mcs.host_att_guest_def + mcs.guest_att_host_def <= 2.75
			and mcs.host_att_guest_def > 0.75 * mcs.guest_att_host_def
			and mcs.host_att_guest_def < 1.33 * mcs.guest_att_host_def
		or mcs.host_att_guest_def + mcs.guest_att_host_def <= 2.35
			and mcs.host_att_guest_def > 0.6 * mcs.guest_att_host_def
			and mcs.host_att_guest_def < 1.66 * mcs.guest_att_host_def)
	  -- and mpk.current_h_win <= 0.96
      and abs((mpk.main_pk-(mpk.main_h_win-mpk.main_a_win)/2) - mp.predict_pk) < 0.15
      and abs(mp.predict_pk) < 0.8
      and m.league not in ('OuLian', 'OuGuan')
      and cal_phase = 2
     order by m.match_time desc;
